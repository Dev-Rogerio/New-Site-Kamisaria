...
app.get("/", (req, res) => {
    res.send("Bem-vindo à Kamisaria Zanuto!");
});
...
app.post("/send-email", async (req, res) => {
    ...
    // ---------------------------------------------------------------------------------------
    // 1️⃣ Configuração do transporter de e-mail
    ...
    try {
        ...
        // 3️⃣ Enviar WhatsApp para o cliente
        const messageCliente = `Olá ${nome}, seu pedido foi recebido com sucesso! Obrigado por comprar na Kamisaria Zanuto.`;
        await sendMessageWhatsApp(telefone, messageCliente);

        // 4️⃣ Enviar WhatsApp para você (notificação interna)
        const messageInterna = `Novo pedido de ${nome} recebido!`;
        const seuNumero = "5511973418998"; // seu WhatsApp
        await sendMessageWhatsApp(seuNumero, messageInterna);

        // 5️⃣ Resposta para o frontend
        res.status(200).json({
            message: "Pedido enviado por e-mail e WhatsApp com sucesso!",
        });
    } catch (error) {
        console.error("Erro ao processar pedido:", error);
        res.status(500).json({ error: "Erro ao enviar e-mail ou WhatsApp" });
    }
});
